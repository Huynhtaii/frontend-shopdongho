...
const SearchResult = ({ keyword, setShowSearchResult, setKeyword }) => {
    ...
    return (
        <>
            <div className="fixed inset-0 bg-black/50 z-[-1]" onClick={() => setShowSearchResult(false)}></div>
                    ...
                    {!loading && !error && products.length > 0 ? (
                        <div className="flex flex-col gap-1">
                            {products.map((product) => (
                                ...
                                >
                                    <img
                                        src={product.ProductImages?.length > 0 ? product.ProductImages[0].url : "https://via.placeholder.com/40"}
