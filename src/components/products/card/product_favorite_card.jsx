    ...
    return (
        <div className="bg-white px-3 pt-3 rounded-md border-[1px] border-[#e7e7e7]">
            ...
            <div className="mt-3">
                ...
                <p className="text-[#ed1c24] font-[600] text-[18px]">{product.discount_price?.toLocaleString() || product.price?.toLocaleString()}đ</p>
                {product.discount_price && (
                    <div className="flex items-center gap-3">
                        <p className="text-[14px] font-[500] line-through text-[#939393]">{product.price?.toLocaleString()}đ</p>
                        <span className="text-xs text-[#ef5555] bg-[#f9e9e2]">{Math.round((product.price - product.discount_price) / product.price * 100)}%</span>
                    </div>
                )}
            </div>
            ...
            <div className="flex flex-col py-3 text-[14px]">
                ...
                <button className="p-2 rounded-md text-gray-500 flex items-center gap-1 justify-center"
                    onClick={() => addAndRemoveToFavorite(product)}
                ><IoCloseSharp size={20} />Xoá sản phẩm</button>
            </div>
        </div >
    )
}
